// A per-user snapshot of the last loaded data, kept in AsyncStorage so the app
...
// (see lib/vault) — so the snapshot is no more exposed than the key that
// unlocks it. It's only ever written while unlocked, and it's cleared on
// sign-out, account deletion, and whenever a device turns out to be locked.
...
export async function loadCache(userId: string): Promise<CacheSnapshot | null> {
  try {
...
export async function saveCache(userId: string, snapshot: CacheSnapshot): Promise<void> {
  try {
    const stored: Stored = { v: VERSION, ...snapshot };
    await AsyncStorage.setItem(CACHE_KEY(userId), JSON.stringify(stored));
  } catch {
    // Storage full or unavailable — the cache is a nicety, so a failed write
    // is fine to swallow.
  }
}
...
export async function clearCache(userId: string): Promise<void> {
  try {
    ...
    // Nothing to clear, or storage unavailable — either way, harmless.
  }
}
