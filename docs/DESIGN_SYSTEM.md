...
- **Mascot:** the **Apple 🥕** — the exact carrot the brief asked for. Use the
  [`Carrot`](../src/components/ui/Carrot.tsx) component. It renders the emoji
  character, which is that carrot on Apple devices and Google's blunter one
  everywhere else. **It sits still** — the
