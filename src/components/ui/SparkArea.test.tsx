...
describe("buildAreaPath", () => {
  ...
  it("maps values onto the viewBox, padded so the stroke isn't clipped", () => {
    // 0 sits on the floor (y=31), the max touches the ceiling (y=1).
    const paths = buildAreaPath([0, 10]);
    expect(paths?.line).toBe("M0 31 L100 1");
    expect(paths?.area).toBe("M0 31 L100 1 L100 32 L0 32 Z");
  });

  it("draws a flat floor for an all-zero series instead of dividing by zero", () => {
    expect(buildAreaPath([0, 0, 0])?.line).toBe("M0 31 L50 31 L100 31");
  });
});
