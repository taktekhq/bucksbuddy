...
// The second way in to the review. Deliberately a plain row that fetches
// nothing: this screen renders every card it holds on every visit, and the
// review screen reads its own entries when it opens.
function ReviewLink() {
  return (
    <button
      type="button"
    ...
    >
      <ChartColumn className="h-5 w-5 shrink-0 text-carrot" strokeWidth={2} aria-hidden />
      <span className="flex flex-1 flex-col">
        <span className="text-base text-label">Spending review</span>
        <span className="text-xs text-label-secondary">
          Your logged months, charted
        </span>
      </span>
      <ChevronRight
