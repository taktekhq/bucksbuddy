...
export type Category = {
  id: string;
  label: string;
  // A lucide icon's export name (e.g. "Briefcase"), not the component itself:
  // web resolves it against `lucide-react`, native against
  // `lucide-react-native` — same names, two packages — so this stays a plain
  // string here rather than pulling either UI library into the shared core.
  icon: string;
...
// Named `...Name`, not `categoryIcon`, precisely because it returns the icon's
// name rather than a renderable component — each shell wraps this with its
// own icon package lookup (see src/lib/categories.ts's `categoryIcon` on web).
export function categoryIconName(id: string): string {
