...
// A minimal in-memory StoragePort — see cache.test.ts/e2e.test.ts for the
// same helper. Exercises the real (async) interface store.tsx is written
// against rather than jsdom's localStorage.
...
// A stable object whose methods always delegate to the *current* `mock`, so
// reassigning `mock = makeSupabaseMock(...)` mid-test (several tests below
// swap handlers partway through) is picked up without re-rendering the
// provider with a new supabase prop.
const liveSupabase = {
  from: (table: string) => mock.supabase.from(table),
  auth: { signOut: (...args: unknown[]) => mock.supabase.auth.signOut(...args) },
  functions: functionsMock,
} as never;

function setup(handlers: Record<string, Handler> = {}, initialCache: CacheSnapshot | null = null) {
...
describe("StoreProvider / useStore", () => {
  ...
  it("starts locked for a passphrase user and unlocks with the right one", async () => {
    ...
    expect(await storage.get("bb-e2e-pass:u1")).toBe("pw"); // persisted
    expect(result.current.transactions).toHaveLength(1);
  ...
  it("auto-unlocks on load from the device-stored passphrase", async () => {
    ...
    await storage.set("bb-e2e-pass:u1", "pw");
    await waitFor(() => expect(result.current.loading).toBe(false));
    expect(result.current.locked).toBe(false);
  ...
  it("drops a stale stored passphrase and stays locked", async () => {
    ...
    const { result } = setup({ "e2e_keys:select": () => ({ data: row }) });
    await storage.set("bb-e2e-pass:u1", "old-passphrase"); // changed elsewhere
    await waitFor(() => expect(result.current.loading).toBe(false));
    expect(result.current.locked).toBe(true);
    expect(await storage.get("bb-e2e-pass:u1")).toBeNull(); // cleared
  });
  ...
  it("turns encryption on then off on the default tier", async () => {
    ...
    expect(await storage.get("bb-e2e-pass:u1")).toBe("a strong passphrase!");
    ...
    expect(await storage.get("bb-e2e-pass:u1")).toBeNull();
  });
  ...
  it("writes a snapshot after loading so the next start is instant", async () => {
    ...
    await waitFor(() => expect(result.current.loading).toBe(false));
    await waitFor(async () => {
      const raw = await storage.get("bb-cache:u1");
  ...
  it("clears the cache and never caches plaintext while locked", async () => {
    ...
    await waitFor(() => expect(result.current.loading).toBe(false));
    expect(result.current.locked).toBe(true);
    expect(await storage.get("bb-cache:u1")).toBeNull();
  });
  ...
  it("drops the cached snapshot on sign-out", async () => {
    ...
    await waitFor(() => expect(result.current.loading).toBe(false));
    await waitFor(async () => expect(await storage.get("bb-cache:u1")).not.toBeNull());
  ...
  it("blocks writes and key changes while locked", async () => {
    ...
    await act(async () => {
      ...
      results.push(await result.current.addSafeGoldEntry({ is_deposit: true, grams: 1 }));
    });
