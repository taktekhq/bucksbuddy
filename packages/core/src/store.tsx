...
async function rowToTransaction(row: TransactionRow, masterKey: CryptoKey): Promise<Transaction> {
...
async function rowToGold(row: SafeGoldEntryRow, masterKey: CryptoKey): Promise<SafeGoldEntry> {
...
export function StoreProvider({
  ...
  supabase,
  storage,
  // Pre-resolved on-device snapshot, so the first frame shows real numbers
  // instead of zeros while the network read is in flight. Reading it is each
  // shell's job, not this file's: web can read localStorage synchronously
  // (see src/lib/store.tsx) so it passes a real snapshot before first
  // render; a shell that can't read synchronously passes null and simply
  // starts loading, same as a first-ever launch.
  initialCache = null,
  // Called after signOut()/deleteAccount() finish clearing local secrets and
  // ending the session. Optional: web uses it to reset the URL hash so it
  // doesn't stay stuck on the page they signed out from (the actual
  // screen swap is already driven by the auth-state listener elsewhere).
  onSignedOut,
  children,
}: {
  userId: string;
  ...
  const loadData = useCallback(async () => {
    ...
    const { data: profile } = await supabase.from("profiles").select("lbp_per_usd").eq("id", userId).single();
    ...
    const [{ data: txData }, { data: goldData }] = await Promise.all([
      supabase
        ...
        // The cap lives in stats.ts so the daily series knows where the
    ...
    if (!key) {
      ...
      await clearCache(storage, userId);
      setLoading(false);
      return;
    }

    setTransactions(await Promise.all(txRows.map((r) => rowToTransaction(r, key))));
  ...
  useEffect(() => {
    void (async () => {
      const vault = await loadVault(supabase, userId);
      if (vault.status === "unlocked") {
        ...
        const stored = await loadStoredPassphrase(storage, userId);
        const key = stored ? await unlockVault(supabase, userId, stored) : null;
        if (key) {
          masterKey.current = key;
          setLocked(false);
          setPassphrase(stored);
        } else {
          if (stored) await clearStoredPassphrase(storage, userId); // stale (changed elsewhere)
  ...
  }, [userId, loadData, supabase, storage]);
  ...
  useEffect(() => {
    ...
    void saveCache(storage, userId, { transactions, lbpPerUsd, safeGoldEntries });
  }, [userId, storage, loading, locked, transactions, lbpPerUsd, safeGoldEntries]);

  const unlock = useCallback(
    async (pass: string) => {
      const key = await unlockVault(supabase, userId, pass);
      if (!key) return { error: "Wrong passphrase." };
      masterKey.current = key;
      await storeStoredPassphrase(storage, userId, pass);
      setPassphrase(pass);
      setLocked(false);
      await loadData();
      return { error: null };
  ...
  const enableEncryption = useCallback(
    async (pass: string) => {
      ...
      await enablePassphrase(supabase, userId, key, pass);
      await storeStoredPassphrase(storage, userId, pass);
      setPassphrase(pass);
      setE2eMode("passphrase");
      return { error: null };
  ...
  const disableEncryption = useCallback(async () => {
    const key = masterKey.current;
    if (!key) return { error: LOCKED_MSG };
    await disablePassphrase(supabase, userId, key);
    await clearStoredPassphrase(storage, userId);
    setPassphrase(null);
    setE2eMode("default");
    return { error: null };
  }, [userId, supabase, storage]);
  ...
  const signOut = useCallback(async () => {
    ...
    // whoever signs in next. The in-memory key goes too.
    await clearStoredPassphrase(storage, userId);
    await clearCache(storage, userId);
    masterKey.current = null;
    setPassphrase(null);
    ...
    onSignedOut?.();
  }, [userId, supabase, storage, onSignedOut]);
  ...
  const updateTransaction = useCallback(
    async (id: string, tx: NewTransaction) => {
      const key = masterKey.current;
      if (!key) return { error: LOCKED_MSG };
      const enc = await encryptTxValues(key, tx);
      const { data, error } = await supabase
      ...
      setTransactions((prev) => prev.map((t) => (t.id === id ? txInMemory(row, tx) : t)));
      return { error: null };
    },
    [supabase],
  );
  ...
  const setRate = useCallback(
    async (rate: number) => {
      const { error } = await supabase.from("profiles").update({ lbp_per_usd: rate }).eq("id", userId);
      if (error) return { error: error.message };
      setLbpPerUsd(rate);
      return { error: null };
    },
  ...
  const deleteSafeGoldEntry = useCallback(
    async (id: string) => {
      ...
      const { error } = await supabase.from("safe_gold_entries").delete().eq("id", id);
      if (error) {
        setSafeGoldEntries(prev);
        return { error: error.message };
      }
  ...
  const safeTotalCents = useMemo(
    () =>
      transactions.reduce(
        (sum, t) =>
          t.category === SAFE_CATEGORY_ID ? sum + (t.is_income ? -t.amount_usd_cents : t.amount_usd_cents) : sum,
        0,
      ),
    [transactions],
  );
  ...
  const safeGoldGrams = useMemo(
    () => safeGoldEntries.reduce((sum, e) => sum + (e.is_deposit ? e.grams : -e.grams), 0),
    [safeGoldEntries],
  );
