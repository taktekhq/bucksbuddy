// Platform-neutral WebCrypto primitives for BucksBuddy's optional end-to-end
// encryption. Web installs the browser implementation by default; native
// configures react-native-quick-crypto during application bootstrap.
//
...
export type CryptoPort = Pick<Crypto, "getRandomValues" | "subtle">;

let configuredCrypto: CryptoPort | null = null;

/** Select a platform crypto provider. Pass null to return to global WebCrypto. */
export function configureCryptoPort(port: CryptoPort | null): void {
  configuredCrypto = port;
}

function cryptoPort(): CryptoPort {
  const port = configuredCrypto ?? globalThis.crypto;
  if (!port?.subtle) throw new Error("A WebCrypto-compatible provider is required");
  return port;
}
...
function randomBytes(n: number): Uint8Array<ArrayBuffer> {
  return cryptoPort().getRandomValues(new Uint8Array(new ArrayBuffer(n)));
}
...
export function generateMasterKey(): Promise<CryptoKey> {
  return cryptoPort().subtle.generateKey({ name: "AES-GCM", length: 256 }, true, [
    "encrypt",
    "decrypt",
  ]);
}
...
function importAesKey(raw: Uint8Array): Promise<CryptoKey> {
  return cryptoPort().subtle.importKey("raw", ab(raw), { name: "AES-GCM" }, true, [
    "encrypt",
    "decrypt",
  ]);
}
...
async function encryptBytes(key: CryptoKey, bytes: Uint8Array): Promise<string> {
  ...
  const ct = new Uint8Array(
    await cryptoPort().subtle.encrypt({ name: "AES-GCM", iv }, key, ab(bytes)),
  );
...
async function deriveWrapKey(
  ...
  const base = await cryptoPort().subtle.importKey(
    "raw",
...
export async function wrapMasterKey(
  ...
  const raw = new Uint8Array(await cryptoPort().subtle.exportKey("raw", masterKey));
